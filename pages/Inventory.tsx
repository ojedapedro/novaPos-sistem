...
import { Search, Plus, Filter, AlertCircle, X, CheckCircle2, Ban } from 'lucide-react';
...
export const Inventory: React.FC = () => {
  ...
  const [stockFilter, setStockFilter] = useState('Todos');   // Todos, Bajo Stock
  ...
  const filtered = useMemo(() => {
    return products.filter(p => {
        ...
        const matchesStatus = statusFilter === 'Todos' 
            ? true 
            : statusFilter === 'Activo' ? p.active : !p.active;
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 animate-fade-in grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        <button 
                            ...
                            className="w-full py-2 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded-lg transition-colors flex items-center justify-center gap-2"
                        >
                ...
                {filtered.length === 0 ? (
                    ...
                    <tr key={product.id} className="hover:bg-blue-50/50 transition-colors group">
                        ...
                        <td className="p-4 text-gray-600 text-sm">
                            <span className="bg-gray-100 px-2 py-1 rounded text-xs border border-gray-200 group-hover:bg-white">{product.category}</span>
                        </td>
                        ...
                        <div className="flex items-center justify-center gap-2">
                            {product.stock <= product.minStock && (
                                <div className="tooltip relative group/tip">
                                    <AlertCircle size={16} className="text-red-500 cursor-help" />
                                    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/tip:block bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
